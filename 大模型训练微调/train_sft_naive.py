...
class CustomDataArguments(DataTrainingArguments):
    dataset_name: str = field(default="/data0/zejun7/open_source/LLaMA-Factory-main/data/sky_original_dataset.json", metadata={"help": "数据集路径"})
    max_samples: Optional[int] = field(default=100, metadata={"help": "可以设置很小的样本数用于快速调试，默认是None，即使用全部数据集。原始分为训练样本数和验证样本数，这里只设置训练样本数，验证样本数会自动从训练样本数中划分一部分作为验证集"})
...
class CustomTrainingArguments(TrainingArguments):
    ...
    output_dir: str = field(default="save/cloud_sft1", metadata={"help": "模型权重保存路径"})
    ...
    learning_rate: float = field(default=2e-4, metadata={"help": "初始学习率，后续根据学习率策略会变化"})
...
def main():
    parser = HfArgumentParser((ModelArguments, CustomDataArguments, CustomTrainingArguments))
...
'''
朴素版大模型 SFT 微调代码，支持单卡和多卡训练，启动方式如下：
CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --nproc_per_node=4 train_sft_naive.py

等同于旧版启动方式：
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 train_sft_naive.py
